...
3. `international`（国际要闻）：3-5 条，发生在中国大陆以外的事
4. `domestic`（国内动态）：3-5 条，中国大陆公司、政策、市场
5. `big_names`（大佬观点）：1-3 条，知名人物（黄仁勋、马斯克、奥特曼、库克、雷军、余承东、李彦宏等）的言论、表态、采访
...
### 4. 多源命中处理
如果一条新闻在原始素材里出现"另据 X、Y 也报道"，必须在摘要中提及"据财联社、IT之家、TechCrunch 多家媒体报道"，体现交叉验证。
...
### 6. 来源标注
每条新闻必须保留 source 字段。多源命中时 source 填主源，但摘要里要提"据多家媒体报道"
    ...
    try:
        resp = requests.post(
            ...
            json={
                ...
                "max_tokens": 12000,  # v12 提升：从 8000 → 12000，给深度内容空间
...
def render_news_item(item, now=None, validations=None):
    """渲染单条新闻为 HTML（含小标题、摘要、来源时间、验证徽章）"""
    now = now or datetime.now(BJT)
    ...
    # 验证警告（如有）
    summary_html = f'<p style="font-size:{summary_size};color:#3e3e3e;line-height:1.85;margin:0 0 8px;text-align:justify;">{summary}</p>'
    if validations:
