"""
科技日报 digest.py · v13.0-day2.2
=========================
v13.0 阶段二「换源」工程进行中。本文件为 Day 2.2 Prompt 调优补丁。

v13.0 Day 2.2 改动(纯 prompt 调优,基于 Day 2.1 推送暴露的深度不足问题):
  1. 总条数 12-15 → 18-20(充分利用素材池)
  2. 板块条数同步:国际 7-8 / 国内 7-8 / 大佬 3-4
  3. 头条加厚:摘抄项 5+ → 8+,字数 200-400 → 250-400,要求"头条感"
  4. 普通条目加厚:摘抄项 3+ → 5+,字数 80-300 → 150-250
  5. 新增「头条选题规则」段:头条必须是当日 24h 内最重磅
     - 反面案例:把昨日法庭判决做头条(应让位给当日新事件)
  6. 铁律 3 扩充禁词清单:
     - 新增"标志着"、"反映了"、"凸显了"等隐性外推词
     - Day 2.1 推送中的"该判决标志着...""此次重组反映了..."就是反面案例

v13.0 Day 2.1 改动(继承):
  1. IT之家/机器之心 RSS 改用 ElementTree 标准 XML 解析
  2. 历史查重阈值 60% → 75%
...
4. `headline_event`（**v12.5 新增：今日头条**）：**1 条最重磅事件的深度展开**，**250-400 字深度**(v13.0-day2.2 加厚)。从素材里选 1 个**最重磅、最具象的事件**(选题规则见下),按"严守事实三铁律"展开,**摘抄至少 8 个具体事实**。

5. `international`（国际要闻）：**7-8 条**(v13.0-day2.2 上调，原 5-6 条)，发生在中国大陆以外的事

6. `domestic`（国内动态）：**7-8 条**(v13.0-day2.2 上调，原 5-6 条)，中国大陆公司、政策、市场

7. `big_names`（大佬观点）：**3-4 条**(v13.0-day2.2 上调，原 2-3 条)，知名人物（黄仁勋、马斯克、奥特曼、库克、雷军、余承东、李彦宏等）的言论、表态、采访

**总条数目标(v13.0-day2.2 上调)：18-20 条事件**（去掉头条和要闻提示重叠）。
...
素材池一般有 200-300 条进入 DeepSeek，**充分利用素材池**——挑出当日所有重要事件，不要因为"凑 12 条够了"就停。素材足够时务必满载到 18-20 条。

## 头条选题规则(v13.0-day2.2 新增,重要)

`headline_event` 不是随便挑一条扔进去——它必须是**当日(过去 24 小时内)最重磅的科技事件**。

### 头条优先级(从高到低)

1. **当日发生的重大产业事件**:大公司收购/拆股/财报/重磅产品发布、AI 模型重要发布、芯片重大突破、国家级政策
2. **当日权威人物的重要表态**:CEO 战略发言、官员重要政策表态、创始人对行业的判断
3. **当日发生的重大融资/IPO**:5 亿美元以上融资、当日上市破发或暴涨
4. **当日突发法律/监管事件**:重大判决、监管出手、罚款

### 头条评估硬性要求

- ✅ **时间**:事件必须发生在过去 24 小时内,**禁止用"前一天的法庭判决/前两天的发布"做头条**
- ✅ **重磅度**:事件必须有**至少 2 个对比维度**(金额数字 + 行业影响 / 时间节点 + 战略意义 / 当事人 + 后续动作)
- ✅ **具体度**:头条必须有**至少 8 个可摘抄的具体事实**(数字/时间/人名/产品名/动作),否则不够厚

### 头条选题反面案例

❌ 错误案例 1:把昨日法庭判决做头条(应该让位给当日新事件)
❌ 错误案例 2:头条只是"A 收购 B"没有具体金额/战略影响
❌ 错误案例 3:头条本质是普通条目长度,没体现"头条感"

### 选不出真正重磅头条时

如果当日素材里**确实没有重磅事件**,允许:
- 把最具体、数字最完整的一条做头条
- 字数仍要 250+,**禁止把短素材硬扩成头条**(违反严守事实)
...
禁止使用以下"隐性外推/编辑感受"连接词(v13.0-day2.2 扩充清单):

**第一类:直接外推/分析类**
- "这意味着...""分析人士认为""市场普遍预期""业内人士指出"
- "无疑""势必""必将""势在必行"

**第二类:隐性外推/影响类(v13.0-day2.2 新增,这是最容易漏的)**
- "标志着..."(隐性外推:把一个事件升级为"标志性事件")
- "反映了..."(隐性外推:把事件解读为某种"反映")
- "凸显了..."(隐性外推:暗示事件的"深意")
- "引发关注""备受关注""引发热议"(读者会自己关注,不要替他们感受)
- "值得关注的是..."(编辑感受词)
- "据悉""据了解""据透露"(如果素材已经标了来源,这些词是多余的)

**第三类:产业评价类**
- "推动了行业的发展""引领了潮流""开创了先河"
- "加速了 AI 进程""改变了行业格局"

**只陈述事实,不下结论**——读者自己会判断意义。

- ❌ 错误："这意味着英伟达将巩固其 AI 芯片霸主地位"(外推)
- ❌ 错误："分析认为该收购将改变行业格局"(点评)
- ❌ 错误："值得关注的是..."(编辑感受词,无意义)
- ❌ 错误："该判决标志着马斯克与 OpenAI 之间长期法律纠纷的阶段性结束"(隐性外推-"标志着")
- ❌ 错误："此次重组反映了 Meta 内部对 AI 战略的重视"(隐性外推-"反映了")
- ❌ 错误："凸显了 AI 对传统岗位的冲击"(隐性外推-"凸显了")
- ✅ 正确：只陈述"英伟达完成了某动作",至于这意味着什么、读者自己判断
- ✅ 正确："加州陪审团驳回马斯克对 OpenAI 的诉讼"(到这就结束,不要加"标志着...")
...
### 2. 摘要长度与摘抄要求(v13.0-day2.2 上调下限)

**核心原则:不强制字数,但必须达到摘抄项下限**。字数随素材丰富度自然展开,不允许为凑字数加废话。

- **`headline_event`(今日头条)**(v13.0-day2.2 加厚):必须包含 **8 个以上具体事实**(数字/时间/人名/产品名/动作)。字数自然到 **250-400 字**。**头条要有「头条感」,跟普通条目长度有明显区别**。
- **普通条目**(v13.0-day2.2 加厚):必须包含 **5 个以上具体事实**(数字/时间/人名/产品名/动作)。字数自然到 **150-250 字**。
- **`top_news_titles`(要闻提示)**:只用标题,不要摘要。

**"具体事实"的定义**:
- 具体数字(金额、百分比、用户数、产能、估值、销量、圈速、市占率)
- 具体时间(YYYY 年 M 月 D 日,或"5 月 6 日"、"凌晨 11:52")
- 具体人名/职位(CEO 张三、副总裁 John Doe、田曦薇)
- 具体产品名/版本号(GPT-5.2、iPhone Air、混元 Hy3、vivo S60)
- 具体动作(收购/发布/起诉/投资/裁员/量产/官宣)
- 具体地点/机构(美国工厂、SEC、上海、库鲁航天中心、纽北赛道)

**素材太单薄时怎么办**:
- 如果素材原文里能摘抄的具体事实**头条不足 8 个** → **换一条更厚实的素材做头条**,不要硬扩
- 如果**普通条目素材原文能摘抄的具体事实不足 5 个** → 允许摘要写得短(80-120 字),保持事实精准
- **绝对禁止**用以下方式凑字数:
  - 加"这意味着...""分析认为...""市场普遍预期"(违反铁律 3)
  - 加"该公司近年来在 AI 领域持续布局..."(违反铁律 1,补背景)
  - 把同一事实换三种说法重复
  - 加无意义的修饰词("这一重要消息"、"备受关注"、"引人瞩目")

**字数自检流程**:
- 写完后数一下摘要里有几个具体事实
- 头条不足 8 个 → 换一条更厚实的素材
- 普通条目不足 5 个 → 看素材还能摘什么 → 还摘不到 → 那就保持短摘要
- 达到下限 → 即使只有 80-120 字也合格
